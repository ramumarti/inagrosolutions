...
import { getTenantDashboardMetrics } from '@/lib/actions/tenant-dashboard';
import { 
  Building2, 
  Users, 
  Map, 
...
export default function TenantDashboard() {
  ...
  useEffect(() => {
    async function load() {
      if (!tenant) return;
      const res = await getTenantDashboardMetrics();
      if (res.success) {
        setData(res.data);
      }
      setLoading(false);
  ...
  const stats = data?.stats || { totalMembers: 0, totalHa: 0, pendingInvites: 0, activeAlerts: 0, health: { label: '...', score: 100 } };
  const billing = data?.billing || { plan: 'basico', price: 0 };
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          { label: 'Alertas Red', val: stats.activeAlerts, icon: Bell, color: 'text-amber-400', bg: 'bg-amber-500/10' },
          { label: 'Salud de Red', val: stats.health.label, icon: ShieldCheck, color: stats.health.score > 80 ? 'text-emerald-400' : 'text-amber-400', bg: stats.health.score > 80 ? 'bg-emerald-500/10' : 'bg-amber-500/10' }
        ].map((s, i) => (
                 ...
                 <div key={i} className="flex gap-4 items-start group">
                    ...
                    <div>
                       <p className="text-sm font-bold text-white/90">
                         <span className="text-[var(--color-primary)]">{item.user}</span> ha realizado un <span className="text-emerald-400 lowercase">{item.action}</span> sobre <span className="opacity-60">{item.entity}</span>
                       </p>
                  ...
                  <div className="py-12 text-center text-white/20 text-sm font-medium">Sin actividad reciente registrada</div>
               )}
            </div>
        ...
        <div className="space-y-6">
          <GlassCard className="p-6 border-emerald-500/20 bg-emerald-500/5">
             ...
             <div className="flex items-end gap-2 mb-1">
                <span className="text-4xl font-black text-white">{billing.price}€</span>
                <span className="text-xs text-white/40 mb-1">/mes</span>
             </div>
             <p className="text-[10px] text-emerald-400 font-bold mb-6">Plan {billing.plan.toUpperCase()} • 50% Dto Aplicado</p>
