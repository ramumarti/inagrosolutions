...
import { ShieldAlert, Users, CalendarCheck, Map } from 'lucide-react';
...
export default function TechnicianLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!hasRole(['technician', 'tenant_admin'])) {
    return (
      <div className="flex items-center justify-center p-8">
        <GlassCard className="p-12 max-w-md text-center">
          <ShieldAlert className="w-16 h-16 text-yellow-500 mx-auto mb-4 opacity-80" />
          <h2 className="text-xl font-bold text-white mb-2">Acceso Denegado</h2>
          <p className="text-sm text-white/50">Esta sección es exclusiva para asesores y técnicos.</p>
        </GlassCard>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full">
      <div className="px-6 md:px-10 py-6 border-b border-white/5 bg-black/20 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white glow-text">Panel del Técnico</h1>
          <p className="text-sm text-white/50 font-medium">{tenant?.name}</p>
        </div>
      </div>
