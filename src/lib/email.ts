        ...
        <tr>
            <td align="center" style="padding-bottom: 24px;">
                <div style="display: inline-block; background: linear-gradient(135deg, #EF4444, #DC2626, #F472B6); border-radius: 12px; padding: 8px 20px;">
                    <span style="color: #ffffff; font-weight: 800; font-size: 16px; letter-spacing: 0.05em;">✦ IASOLUTIONS</span>
                </div>
            </td>
        </tr>
        <tr>
...
export async function sendWelcomeEmail(options: {
  ...
  try {
    ...
    const html = buildEmailTemplate({
      title: '¡Bienvenido a IASOLUTIONS!',
    ...
    return await sendEmail({
      to: options.to,
      subject: '¡Bienvenido a IASOLUTIONS! Tu acceso está listo',
      html
    });
  } catch (error: unknown) {
    const err = error as Error;
